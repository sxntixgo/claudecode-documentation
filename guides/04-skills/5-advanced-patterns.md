...
Skill output is generated by a model, so it varies between runs even when the skill is working correctly. That rules out the testing techniques you would reach for with ordinary code: exact-string comparison, `diff` against a fixture, and snapshot assertions all fail on harmless rewording while missing real regressions in substance.

What you measure instead is whether the output satisfies a set of stated conditions, and whether the skill improves on what Claude does without it. The `skill-creator` plugin automates that loop:
...
Then ask Claude to evaluate a skill by name, for example `evaluate my code-review skill with skill-creator`.

---

### Pattern 7: Baseline Comparison

**Goal**: Prove the skill is better than no skill

A skill that produces good output has not necessarily earned its tokens — Claude may have produced equally good output without it. The only way to know is to run both.
...
```json
{
  ...
  "evals": [
    {
      "id": 1,
      "prompt": "Review the staged changes for security issues",
      "expected_output": "Flags the unparameterized SQL query and the missing auth check",
      "files": ["fixtures/handlers.ts"]
    }
  ]
}
```

For each case, two subagents run in the same turn — one with the skill available, one without. Each starts from a clean context, which matters because leftover context from authoring the skill will mask gaps in the written instructions. Results are written to `with_skill/outputs/` and `without_skill/outputs/`.

Run cases in a fresh session. If you test in the session where you wrote the skill, you are measuring your conversation, not your SKILL.md.

---

### Pattern 8: Assertion-Based Grading

**Goal**: Score output against conditions rather than exact text

Grading runs in two phases. Deterministic checks verify observable facts — which tools were called, which files were written, whether a command ran, whether output stayed inside a token budget. A rubric phase then scores the remaining qualities that need judgment, using a model as the grader against criteria such as accuracy, completeness, clarity, and formatting.

Write assertions that are objectively verifiable and named so they read clearly in a report:

```json
{
  "eval_id": 0,
  "eval_name": "flags-sql-injection-in-handler",
  "prompt": "Review the staged changes for security issues",
  "assertions": [
    "Identifies the unparameterized query on line 42",
    "Recommends parameterized queries or an ORM method",
    "Does not flag the already-parameterized query on line 88"
  ]
}
```

That last assertion is worth copying as a habit. False positives make a review skill tiring to use, and they only show up if you assert on them.

Results land in `grading.json` with `text`, `passed`, and `evidence` per assertion, so a failure tells you what the output actually said rather than just that it differed.

---

### Pattern 9: Trigger Accuracy

**Goal**: Verify the skill fires when it should, and stays quiet when it should not

Output quality only matters if the skill activates. These are separate failure modes, and a skill can pass every eval while never triggering in real use.

As your skill count grows, description precision decides this. Too broad and unrelated requests pull the skill in; too narrow and it never fires. The plugin generates should-trigger and should-not-trigger prompts, measures the hit rate on each, and proposes description edits when activation lands on the wrong requests.

Anthropic's guidance is to make descriptions slightly pushy, because Claude tends to underuse skills that would have helped:

```yaml
# Triggers reliably — states the action and the moment
description: Reviews staged git changes for security, logic, and style issues before commit.

# Triggers unpredictably — no action, no trigger condition
description: Helps with code review
```

---

### Benchmarking and Version Comparison

Benchmark mode aggregates a full eval set into `benchmark.json`, recording pass rate, elapsed time, and token count for with-skill against without-skill. That pairing is the point: a skill that lifts pass rate from 60% to 95% is worth its tokens, and one that lifts it from 91% to 93% for triple the tokens probably is not.

Before committing a change to a skill, run a blind A/B between the two versions. This is the gap that semantic versioning alone leaves open — a changelog records that you changed something, not that the change was an improvement.

**Test against every model you plan to run the skill on.** Instructions that are sufficient for Opus often underspecify for Haiku, and content that Opus finds redundant may be exactly what Haiku needs.
