# Community Resources & Learning

**Reading Time**: 20 minutes
**Skill Level**: All Levels
**Last Updated**: 2025-01-15

---

## Overview

This guide catalogs official resources, community contributions, and learning materials for Claude Code and related technologies.

---

## Official Anthropic Resources

### Claude Code Documentation

- **Official Docs**: https://code.claude.com/docs
- **GitHub Repository**: https://github.com/anthropics/claude-code
- **Issue Tracker**: https://github.com/anthropics/claude-code/issues
- **Release Notes**: https://github.com/anthropics/claude-code/releases

### Claude API & Platform

- **Claude API Docs**: https://docs.anthropic.com
- **API Reference**: https://docs.anthropic.com/en/api
- **Model Cards**: https://www.anthropic.com/claude
- **Pricing**: https://www.anthropic.com/pricing
- **System Status**: https://status.anthropic.com

### Model Context Protocol (MCP)

- **Official Website**: https://modelcontextprotocol.io
- **Specification**: https://spec.modelcontextprotocol.io
- **GitHub**: https://github.com/modelcontextprotocol
- **SDK Documentation**: https://github.com/modelcontextprotocol/typescript-sdk

---

## Community Resources

### Skills Repositories

**Official Anthropic Skills**:
- Repository: https://github.com/anthropics/skills
- Curated collection of official skills
- Production-ready examples
- Best practices demonstrations

**Community Skills (obra/superpowers)**:
- Repository: https://github.com/obra/superpowers
- Large community skill collection
- Diverse use cases
- Active contributions

### MCP Servers

**Official Servers**:
- https://github.com/modelcontextprotocol/servers
- File system, database, API integrations
- Reference implementations

**Community Servers**:
- Search "MCP server" on GitHub
- Third-party integrations
- Specialized tools

---

## Learning Resources

### Video Tutorials

**Getting Started**:
- Anthropic YouTube Channel: https://youtube.com/@anthropic-ai
- Claude Code intro videos
- Feature walkthroughs

**Community Channels**:
- Search "Claude Code tutorial" on YouTube
- Developer walkthroughs
- Project showcases

### Blog Posts & Articles

**Anthropic Engineering Blog**:
- https://www.anthropic.com/engineering
- Technical deep dives
- Feature announcements
- Best practices

**Community Blogs**:
- Dev.to: https://dev.to/t/claude
- Medium: Search "Claude AI development"
- Personal developer blogs

### Interactive Learning

**Prompt Engineering**:
- https://docs.anthropic.com/en/docs/prompt-engineering
- Techniques and strategies
- Examples and anti-patterns

---

## Community Channels

### Official Support

**Discord** (if available):
- Official Anthropic Discord
- Community support
- Feature discussions

**GitHub Discussions**:
- https://github.com/anthropics/claude-code/discussions
- Q&A format
- Feature requests
- Showcase projects

### Social Media

**Twitter/X**:
- @AnthropicAI - Official updates
- #ClaudeCode - Community posts
- Follow developers sharing tips

**Reddit**:
- r/ClaudeAI - Community discussions
- r/artificial - Broader AI development
- r/MachineLearning - Technical discussions

---

## Contributing

### Contributing to Documentation

**This Documentation**:
- See [CONTRIBUTING.md](../../../CONTRIBUTING.md)
- Submit issues and pull requests
- Improve examples
- Fix errors

### Contributing Skills

**Creating Skills**:
1. Follow [Skill Creation Guide](../../3-skills/2-creating-skills.md)
2. Test thoroughly
3. Document usage
4. Submit to community repos

### Contributing MCP Servers

**Building Servers**:
1. Follow MCP specification
2. Include comprehensive tests
3. Document installation and usage
4. Publish to npm/PyPI

---

## Developer Tools

### IDEs and Extensions

**VS Code**:
- Claude Code VS Code extension
- MCP Server development tools
- Markdown preview for documentation

**JetBrains**:
- Check for Claude Code plugins
- Community integrations

### Testing Tools

**MCP Testing**:
- @modelcontextprotocol/sdk - Official testing utilities
- Vitest for unit tests
- Jest for integration tests

**Skill Testing**:
- Local testing with Claude Code CLI
- Automated testing frameworks

---

## Example Projects

### Open Source Projects Using Claude Code

**Search GitHub**:
- Topic: "claude-code"
- Language filters
- Star sorting

**Example Searches**:
- https://github.com/topics/claude-code
- https://github.com/search?q=claude+code+skill
- https://github.com/search?q=mcp+server

### Project Templates

**Starter Templates**:
- See [Examples Section](../../9-examples/1-overview.md)
- React + TypeScript
- Node.js API
- Django
- FastAPI
- Flask

---

## Staying Updated

### Release Monitoring

**GitHub Watch**:
- Watch https://github.com/anthropics/claude-code
- Get notifications for releases
- Track issue discussions

**RSS Feeds**:
- Anthropic blog RSS
- GitHub release feeds
- Community blog aggregators

### Newsletters

**AI/ML Newsletters**:
- The Batch (deeplearning.ai)
- Import AI
- Last Week in AI

---

## Getting Help

### Troubleshooting Resources

1. **Check Documentation**:
   - [Troubleshooting Guide](../../10-reference/2-troubleshooting.md)
   - [FAQ](../../10-reference/3-faq.md)

2. **Search GitHub Issues**:
   - Similar problems and solutions
   - Known bugs and workarounds

3. **Ask the Community**:
   - GitHub Discussions
   - Discord/Reddit
   - Stack Overflow (tag: claude-ai)

4. **Official Support**:
   - For Claude API issues: support@anthropic.com
   - For critical bugs: GitHub Issues

---

## Contributing Back

### Ways to Help

**Documentation**:
- Fix typos and errors
- Add missing examples
- Improve explanations
- Translate to other languages

**Code**:
- Submit bug fixes
- Create new skills
- Build MCP servers
- Develop tools and integrations

**Community**:
- Answer questions
- Share your projects
- Write tutorials
- Record screencasts

### Recognition

**Hall of Fame**:
- Top contributors
- Notable projects
- Community champions

---

## Related Guides

- [Getting Started](../../README.md)
- [CONTRIBUTING Guide](../../../CONTRIBUTING.md)
- [Skill Creation](../../3-skills/2-creating-skills.md)
- [MCP Overview](../../1-mcp-servers/1-overview.md)

---

**Maintained By**: Documentation Team
**Community Contributions**: Welcome!
